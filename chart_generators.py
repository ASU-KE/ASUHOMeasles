   ...
   # Prepare data
    ...
    fig.update_layout(
        geo=dict(
            ...
            domain=dict(x=[0, 1.0], y=[0.15, 0.85])  # Better vertical positioning
        ),
        plot_bgcolor='white',
        paper_bgcolor='white',
        ...
        width=1000,
        height=800,
        margin=dict(l=20, r=20, t=20, b=100)  # Reduced top margin, adequate bottom
    )
    ...
    # Add timestamp and notes positioned in the footer area
    fig.add_annotation(
...
def create_lives_saved_chart(vaccine_impact_data):
    ...
    fig.update_layout(
        ...
        width=1000,
        height=600,
        margin=SPACING['margin'], # Applied margin spacing
        xaxis=dict(
    ...
    legend_x = 0.0  # Start legend at the left
    legend_y = SPACING['legend_y'] # Position higher
    cell_height = 0.03
    ...
    # Add timestamp in bottom left - Adjusted position to match first graph's footer position
    fig.add_annotation(
        text=f"<b>Last refreshed:</b> {datetime.now().strftime('%B %d, %Y at %I:%M %p')}",
        xref="paper", yref="paper",
        x=0.0, y=SPACING['footer_y'], # Applied footer y position
        showarrow=False,
